...
After the bounded test, compare `.proof.ndjson` transitions, per-feed desired/sent/received evidence and source/session files. Require actual accepted/written trade AND quote events, no diagnostics from the actual reader for the bounded interval, no drops or budget violations, unchanged protected groups, and withdrawal evidence. Written counts and complete BAR continuity are NEVER inferred from `connected`, sent counts or the controller receipt. Fable independently reads and hashes the outputs. The raw reader now uses a common reception cutoff W and a target of16384 records, with atomic cursor/receipts per page and a frozen joint file-size snapshot across each drain cycle. Drain admission stops after16 pages or500ms; a page already being read/committed is not preempted by that timer. Actual receipt clocks are retained; W is never used to backdate knowledge. Non-ticks and quarantined frames have hash/range/code receipts. Out-of-order reception has counted notices, not a global gap. Only successfully decoded events increment each file's event sequence. Source cursor offsets cover all receipted frames. **Activation still requires exact-head GO.** Fable disposition rev1a493e40ef qualifies equivalence to a matching schedule of receipt-cut snapshots, convergence by fixed per-file shares and16384 as a target; the named structural properties are covered by tests. See `R2D2_V2_RECEPTION_PAGINATION.md`. Retain all epoch raw files and journal: no manual raw purge during the epoch. The session-directory limit is250, file limit4096 remains a capacity gate.
...
F408b-1 corrects the prior cumulative LIVE log exhaustion. Tests cover a full
16-entry withdrawal queue, compact byte bounds across90 synthetic days, UTC
rollover, restart with an old full file, unchanged PROOF evidence and fail-closed
append limits. JSON parsing failures are separately reported as
`LIVE_POLICY_OR_RELEASE_JSON_INVALID` without exposing their contents. Optional
