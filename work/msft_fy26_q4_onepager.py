...
OUT = Path("outputs/msft-one-pager-fy26-q4.pdf")
...
valuation_methods = [
    ("Goldman Sachs", 599.81),
    ("Morgan Stanley", 627.90),
    ("Bridgewater", 633.49),
    ("JPMorgan", 631.14),
    ("BlackRock", 616.66),
]
...
def metric_table(c, x, y, w, h):
    ...
    section_title(c, "FY26 Q4 - resultado reportado", x + 11, y + h - 20, w - 22)
    columns = [x + 12, x + 170, x + 254, x + 337]
    header_y = y + h - 46
    c.setFillColor(LIGHT)
    c.rect(x + 9, header_y - 4, w - 18, 16, fill=1, stroke=0)
    for i, text in enumerate(("Métrica", "FY26 Q4", "FY25 Q4", "YoY / delta")):
        c.setFillColor(SUB)
        c.setFont("Helvetica-Bold", 6.4)
        c.drawString(columns[i], header_y + 1, text.upper())

    rows = [
        ("Receita consolidada", "US$ 90,0 bi", "US$ 76,4 bi", "+18%"),
        ("Lucro operacional", "US$ 40,6 bi", "US$ 34,3 bi", "+18%"),
        ("EPS non-GAAP", "US$ 4,74", "US$ 3,86", "+23%"),
        ("Microsoft Cloud", "US$ 59,3 bi", "US$ 46,7 bi", "+27%"),
        ("Azure - crescimento", "+43%", "+39%", "+4 p.p."),
        ("Commercial RPO", "US$ 678 bi", "-", "+84%"),
    ]
    row_y = header_y - 14
...
def valuation_table(c, x, y, w, h):
    ...
    summaries = [
        ...
        ("CONSENSO", f"US$ {consensus:.2f}", "57 analistas", INK),
...
def scenario_box(c, x, y, w, h):
    ...
    scenarios = [
        ("BEAR", "US$ 480", f"{(480 / price - 1) * 100:+.0f}%", RED, colors.HexColor("#FFF1F1"), colors.HexColor("#F2B8B5")),
        ("BASE", f"US$ {target:.0f}", f"+{upside:.0f}%", BLUE, BLUE_LIGHT, colors.HexColor("#A9D3EE")),
        ("BULL", "US$ 720", f"+{(720 / price - 1) * 100:.0f}%", GREEN, GREEN_LIGHT, colors.HexColor("#A9D9C4")),
    ]
    cards_x = x + 12
    cards_y = y + 67
    cards_w = w - 24
...
def draw():
    ...
    c.setTitle("MSFT One Pager - FY26 Q4")
    ...
    c.drawString(M, PAGE_H - 50, "MSFT | MICROSOFT")
    c.setFillColor(colors.HexColor("#CAD1D8"))
    c.setFont("Helvetica", 8)
    c.drawString(M, PAGE_H - 66, "FY26 Q4: Azure acelera para 43%; Copilot escala; capex de IA segue elevado")
    ...
    bullet_box(
        ...
        [
            "Receita de US$ 90,0 bi cresceu 18%, enquanto Azure acelerou para 43%, acima do ritmo de 39% observado um ano antes.",
            "Microsoft Cloud atingiu US$ 59,3 bi e o commercial RPO chegou a US$ 678 bi, reforçando a visibilidade de receita plurianual.",
            "O mix segue deslocando o grupo para cloud e IA; More Personal Computing recuou 4%, mas representa parcela menor do lucro incremental.",
        ],
        accent=GREEN,
    )
    bullet_box(
        ...
        [
            "EPS non-GAAP avançou 23%, acima da receita, refletindo escala operacional mesmo com a expansão acelerada de infraestrutura.",
            "Fluxo de caixa operacional foi de US$ 55,4 bi, mas o FCF ficou em US$ 19,6 bi devido ao capex trimestral de US$ 41 bi.",
            "Margem bruta de cloud permaneceu perto de 67% e cedeu cerca de 2 p.p.; a monetização da nova capacidade será o principal teste do FY27.",
        ],
        accent=RED,
    ...
    bullet_box(
        ...
        [
            "Azure combina crescimento acima de 40%, demanda superior à capacidade e backlog recorde, sustentando crescimento de dois dígitos no FY27.",
            "Copilot já supera 30 milhões de assentos pagos e amplia ARPU, retenção e monetização da base de Microsoft 365 e GitHub.",
            "A integração entre Azure, OpenAI, Fabric, Security e aplicativos cria uma distribuição difícil de replicar e reduz o custo de aquisição.",
            "Nosso TP de US$ 622 preserva prêmio de qualidade, mas exige conversão disciplinada do capex em receita, margem e fluxo de caixa livre.",
        ],
        accent=BLUE,
    )
    bullet_box(
        ...
        [
            "Capex do calendário 2026 foi indicado perto de US$ 175 bi; atrasos de monetização comprimiriam o FCF e o múltiplo justo.",
            "Guidance FY27 Q1: receita de US$ 89,85-90,95 bi, Azure perto de 45% em moeda constante e capex acima de US$ 50 bi.",
            "Monitorar margem bruta de cloud, crescimento do RPO, utilização da capacidade, adoção de Copilot e disciplina de despesas.",
            "Riscos adicionais incluem concorrência em IA, dependência da OpenAI, regulação antitruste e desaceleração empresarial.",
        ],
        accent=RED,
        fill=colors.HexColor("#FFFAFA"),
    )
    ...
    footer = (
        "<b>Fontes:</b> Microsoft FY26 Q4 Earnings Release e earnings call; StockAnalysis, MarketBeat, TipRanks e Investing.com para cobertura pública. "
    ...
    c.drawRightString(PAGE_W - M, 13, "Atualizado em 01/08/2026 | Horizonte: 12 meses")
