...
| 8 | pendente ~1/09 | H3 do estudo de entrada: os decis do composite canônico separam retorno forward? (condição 8 da spec: insumo declarado do business case V3) + eixos por sub-score. | runner #257, leitura 5ª sessão | SE o composite não separa: o argumento central do V3 ganha baseline quantificado, e os eixos dizem qual sub-score trocar primeiro. A linha mais importante deste ledger. | LIDO_OFFLINE_INSUFICIENTE(07/09/2026): dado produzido offline pelo runner congelado sobre a população fechada (643 BUYs; época corrente 264 em 8 sessões) — H3 INSUFFICIENT_SAMPLE pelas regras (8/15 sessões, 25/30 decididas por célula; nunca atingível com a v1 morta); leitura descritiva: decil superior do composite p̂ 0,48 vs inferior 0,56, +60 min −0,26 % vs −0,31 %, sem monotonicidade nos decis nem nos sub-scores. Evidência: leitura `85c35a49…`, report `f6bce4ae…`, manifest `2301f17d…`; recibo do dono `e3b436bd…` |
...
| 10 | 07/09 | Leitura do gate V3 (13 noites de shadow, 26/08→07/09, streak 13, soak_eligible desde 04/09): gates de promoção (p50 ≤ 15 % / p90 ≤ 30 % internos) REPROVADOS em 39/39 mercados-noites e em todos os perfis com n ≥ 10 (07/09: B3 19,4 %/47,9 %; NASDAQ 33,8 %/66,8 %; NYSE 24,1 %/57,7 %); viés mediano negativo em todos; V3 ≈ V2 na régua interna (p90 melhora 1–5 p.p.). Macro reutilizada de 24/08 nas 13 noites (G3). Deliberação a seis mãos de 07/09: shadow mantido, sem promoção. | leitura `07417723…` (final, 13 noites) / conferência Codex 12 noites `471f955a…` / deliberação `fe2f86a3…` / recibo do dono `97ef0daa…` | Mandado V3.2 aberto (spec própria): pesos por erro histórico + desduplicação (#9), re-execução B3 (#4), não-engate do V3.1 no B3 (`not_peer_anchor` 84/306), beta negativo zerado nos EUA (116/73), política de renovação macro do shadow. Travas consumer/TP mantidas. | ABERTO |
