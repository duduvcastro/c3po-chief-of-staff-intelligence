...
| 9 | 28/08 | TP canônico dos one-pagers = média aritmética simples dos 5 métodos, incluindo o "Blend Ajustado ao Risco" que já é mistura dos demais (contagem dupla). Constatado pelo Dudu em painel real (TP 107,13 = média exata ao centavo). | one_pager.py:589 (`statistics.mean`) no commit vigente | O trono que o V3 disputa é uma média simples com contagem dupla; se o V3 reprovar no gate, "pesos por erro histórico + remoção da contagem dupla" é o item 1 do mandado V3.2. | ABERTO |
