...
Todo leitor recebe `fetched_before` (o corte `C`) e resolve **uma única safra** `S` = manifesto com o maior `fetched_at < C` (regra única da spec). Para um símbolo, a série servida é "última linha com `fetched_at ≤ fetched_at(S)`" dentro da janela de `S`, **verificada contra o `series_sha256` que `S` gravou** para esse símbolo:
...
`label_bar(mercado, símbolo, sessão, h, fetched_before)` localiza a sessão `h` pregões à frente no calendário da bolsa (`exchange_calendars` 4.13.2: XNYS para NASDAQ/NYSE, BVMF para B3 — a spec grafa `XBSP`; errata documental para uma rev futura) e devolve `not_a_session`/`beyond_calendar` (calendário), `not_yet_mature` **enquanto o fecho da sessão alvo (hora da bolsa, não a data civil) não for anterior ao corte**, o status da safra quando ela recusa o símbolo, `missing_bar` (a bolsa teve sessão, a safra tem o símbolo e não tem a barra) ou `labelled` com `close`, `adjusted_close`, `bar_sha256`, `fetched_at` e `price_snapshot_id` — nunca um preço de outra fonte.
...
- **OFF por padrão.** A fase noturna (`OffhoursPhase("price_history")`) só entra no worker de valuation com `C3PO_VALUATION_PRICE_HISTORY_ENABLED=true`; o cartão de saúde só a espera quando ligada.
