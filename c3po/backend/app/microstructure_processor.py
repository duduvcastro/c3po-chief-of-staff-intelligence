...
class MicrostructureProcessor:
    ...
    def _process_trade(
        ...
        volume = max(0.0, self._float(item.get("v"), 0.0))
        ...
        for aggregate in self._aggregates_for(symbol, event_at, aggregates):
            ...
            if bbo_age_ms is not None:
                ...
                aggregate.max_bbo_age_ms = bbo_age_ms if aggregate.max_bbo_age_ms is None else max(aggregate.max_bbo_age_ms, bbo_age_ms)
            if interarrival_ms is not None:
                aggregate.interarrival_ms_sum += interarrival_ms
                aggregate.interarrival_count += 1
                aggregate.min_interarrival_ms = interarrival_ms if aggregate.min_interarrival_ms is None else min(aggregate.min_interarrival_ms, interarrival_ms)
                aggregate.max_interarrival_ms = interarrival_ms if aggregate.max_interarrival_ms is None else max(aggregate.max_interarrival_ms, interarrival_ms)
    ...
    def _classify(
        ...
        if bbo and timedelta(0) <= event_at - bbo.as_of <= self.bbo_max_age:
            midpoint = (bbo.bid + bbo.ask) / 2
            age_ms = (event_at - bbo.as_of).total_seconds() * 1_000
            if price > midpoint:
                confidence = 1.0 if price >= bbo.ask else 0.8
                return "buy", "bbo_midpoint", confidence, age_ms
            if price < midpoint:
                confidence = 1.0 if price <= bbo.bid else 0.8
                return "sell", "bbo_midpoint", confidence, age_ms
        if previous:
            if price > previous[0]:
                return "buy", "tick_rule", 0.6, None
            if price < previous[0]:
                return "sell", "tick_rule", 0.6, None
            if previous[1] in {"buy", "sell"}:
                return str(previous[1]), "inherited_tick", 0.4, None
