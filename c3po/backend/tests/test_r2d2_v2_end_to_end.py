...
from app.r2d2_v2_sources import FileShadowSource, MANIFEST_SHA, canonical
...
def rig(tmp_path):
    ...
    for name, risk in (("SYNTHA", 20), ("SYNTHB", 80)):
        ...
        rows.append(dict(symbol=name, market="NASDAQ", security_type="COMMON_STOCK", classification_verified=True,
            ...
            quote=dict(bid=99.95, ask=100.05, bid_source_at=stamp, ask_source_at=stamp, available_at=stamp),
    ...
    snapshot = dict(schema="V2_SHADOW_SOURCE_SNAPSHOT_V1", manifest_sha=MANIFEST_SHA,
    ...
    collector = ShadowCollector(store, FileShadowSource(root), release, calendar=calendar)
    return collector, store, root, snapshot, now


def event_file(root, event, sequence):
    body = dict(schema="V2_SHADOW_SOURCE_EVENT_V1", manifest_sha=MANIFEST_SHA, source_id="synthetic-events",
...
def test_two_arms_first_partial_bar_and_restart_have_single_receipts(rig):
    ...
    restart = ShadowCollector(store, FileShadowSource(root), collector.release, calendar=collector.calendar)
...
def test_cohort_export_retains_zero_days_and_does_not_hide_missing_session(rig):
    ...
    final = collector.calendar.details(collector.schedule[28])["close"]
    collector.cycle(final)
    state = store.read(EPOCH)["state"]
    result = export_cohort(state, 20, now=final, calendar=collector.calendar)
    assert len(result["sessions"]) == 20 and result["data_gate_unknown"]
    ...
    with pytest.raises(ShadowIntegrityError, match="COHORT_NOT_MATURE"):
        export_cohort(state, 30, now=final, calendar=collector.calendar)
